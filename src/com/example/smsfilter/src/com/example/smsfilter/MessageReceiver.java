...
public class MessageReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context arg0, Intent arg1) {
		...
		SmsMessage msg = SmsMessage.createFromPdu((byte[]) pdus[0]); // read
																		// incoming
																		// sms
																		// message
																		// and
		...
		try {
			...
			int duration = Integer.parseInt(appSp.getString(
					MessageReceiver.duration, ""));
			...
			// if the sender exists and was blocked or not yet expired then
			// prevent message from being stored
			if (msg.getOriginatingAddress().contains(name) && !val
					&& duration != 0) {
				// delete the sms
				abortBroadcast();
				ed.putString(MessageReceiver.duration, new String(""
						+ (duration - 1)).toString()); // reduce the duration by
														// 1 each time we get
														// the sms
				ed.commit();
				Toast.makeText(
						arg0,
						"Message From: " + msg.getOriginatingAddress()
								+ " was Trashed.", Toast.LENGTH_LONG).show();

			} else if (msg.getDisplayMessageBody().contains(bad_msg)
					&& duration != 0) {
				abortBroadcast();
				ed.putString(MessageReceiver.duration, new StringBuilder(""
						+ (duration - 1)).toString()); // reduce the duration by
														// 1 each time we get
														// the sms
				ed.commit();
				Toast.makeText(arg0, "Bad Message. SMS Filtered",
						Toast.LENGTH_LONG).show();
			} else
				Toast.makeText(arg0,
						"Filter Free Message. No Further Action Required",
						Toast.LENGTH_LONG).show();

		} catch (Exception ex) {
			Toast.makeText(arg0, "Error Message:  " + ex.getMessage(),
					Toast.LENGTH_LONG).show();
		}
